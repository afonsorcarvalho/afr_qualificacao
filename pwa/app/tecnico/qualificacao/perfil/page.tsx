  ...
  return (
    <div className="mx-auto w-full max-w-[880px] space-y-4">
      ...
      <section className="rounded-2xl border border-border/70 bg-muted p-5">
        <div className="flex items-center gap-3">
          {companyLogo ? (
            ...
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-border bg-muted text-lg font-bold text-foreground">
              {initials}
            </div>
          )}
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Técnico</p>
      ...
      <section className="rounded-xl border border-border/70 bg-muted/20 p-3">
        <h2 className="mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Identidade</h2>
        <ul className="divide-y divide-border/40">
          <Row icon={<UserIcon className="h-4 w-4 text-muted-foreground" />} label="Nome" value={userName || '—'} />
          <Row icon={<Mail className="h-4 w-4 text-muted-foreground" />} label="Email" value={email ?? '—'} />
          <Row icon={<Building2 className="h-4 w-4 text-muted-foreground" />} label="Empresa" value={companyName || '—'} />
          <Row icon={<Server className="h-4 w-4 text-muted-foreground" />} label="Servidor" value={serverUrl || '—'} mono />
          <Row icon={<Database className="h-4 w-4 text-muted-foreground" />} label="Database" value={dbName || '—'} mono />
        </ul>
      </section>

      {availableCompanies.length > 1 && (
        <section className="rounded-xl border border-border/70 bg-muted/20 p-3">
          <h2 className="mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Empresa ativa</h2>
            ...
            {companyOpen && (
              <div className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-lg border border-border/70 bg-background shadow-2xl">
                <button
                  ...
                  className={`block w-full px-3 py-2 text-left text-sm ${selectedCompanyId === null ? 'bg-info-surface text-info' : 'text-foreground/80 hover:bg-muted/40'}`}
                >
                  Todas as empresas
                </button>
                {availableCompanies.map((c) => (
                  <button
                    ...
                    className={`block w-full px-3 py-2 text-left text-sm ${selectedCompanyId === c.id ? 'bg-info-surface text-info' : 'text-foreground/80 hover:bg-muted/40'}`}
                  >
                    {c.name}
                  </button>
                ))}
      ...
      <section className="rounded-xl border border-border/70 bg-muted/20 p-3">
        <h2 className="mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Aparência</h2>
        <button
      ...
      <Button
        variant="outline"
        className="min-h-[48px] w-full border-danger/40 text-danger hover:bg-danger/10"
